...
Buffers can create by using one of the static `link:/es4x/@vertx/core/classes/buffer.html#buffer[Buffer.buffer]` methods.
...
Use `link:/es4x/@vertx/core/classes/buffer.html#length[length]` to obtain the length of the buffer.
The length of a buffer is the index of the byte in the buffer with the largest index + 1.

=== Copying buffers

Use `link:/es4x/@vertx/core/classes/buffer.html#copy[copy]` to make a copy of the buffer

=== Slicing buffers

A sliced buffer is a new buffer which backs onto the original buffer, i.e. it does not copy the underlying data.
Use `link:/es4x/@vertx/core/classes/buffer.html#slice[slice]` to create a sliced buffers
